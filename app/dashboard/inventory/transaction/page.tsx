...
export default function TransactionPage() {
  ...
  const handleCreateTransactions = async () => {
    ...
    if (successCount > 0) {
      toast({
        title: "Success",
        description: `Created ${successCount} transaction${successCount > 1 ? 's' : ''}${failureCount > 0 ? ` (${failureCount} failed)` : ''}`,
        variant: "default",
      });
    } else if (failureCount > 0) {
      toast({
        title: "Error",
        description: `Failed to create ${failureCount} transaction${failureCount > 1 ? 's' : ''}`,
        variant: "destructive",
      });
    }
