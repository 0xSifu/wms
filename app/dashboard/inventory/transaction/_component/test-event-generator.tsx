...
export function TestEventGenerator() {
  ...
  const generateEvent = async () => {
    try {
      ...
      if (createdTransactions.length > 0) {
        try {
          ...
          toast({
            ...
            description:
              'Transactions were saved but could not be added to a batch',
            variant: 'destructive'
          });
        }
      }
      ...
      toast({
        title: `${successCount} Transaction${
          successCount !== 1 ? 's' : ''
        } Created`,
        description: `Successfully created ${successCount} transaction${
          successCount !== 1 ? 's' : ''
        } with EPC: ${epc}`,
    ...
    <Card className="w-full">
      ...
      <CardContent>
        <div className="grid gap-4">
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            />
            <Button variant="outline" onClick={handleGenerateRandomEpc}>
              Random
            </Button>
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input
              ...
              className="col-span-2"
            />
            <Button variant="outline" onClick={handleGenerateRandomRssi}>
              Random
            </Button>
          </div>
          <div className="grid grid-cols-4 items-center gap-4">
      ...
      <CardFooter className="flex justify-between">
        <Button onClick={generateEvent} disabled={isLoading}>
          {isLoading
            ? 'Generating...'
            : `Generate ${count > 1 ? count + ' Events' : 'Event'}`}
        </Button>
        <Button
          variant="outline"
          onClick={() => {
